...
####################################################################
    ...
    @staticmethod
    def get_tile_config_candidates(numel) -> List[int]:
        factors = []
        ...
        for i in range(1, sqrt_n+1):
            if numel % i ==0:
                factors.append(i)
                if i*i !=numel:
                    factors.append(numel // i)
        
        #2의 지수승에 대해서 추가해준다.(단 numel보다는 작은 지수승을 가지고 온다.)
        factors = set(factors)
...
####################################################################  
  ...
  ##############################################################################  
        ...
        # XBLOCK을 xnumel의 약수 and 2의 지수승으로 정하는 걸 자동으로 하고
        # YBLOCK도 마찬가지로 ynumel의 약수 and 2의 지수승으로 정하는 걸 자동으로 하는게 목표
        # RBLOCK은 일단 기존과 마찬가지로 수행하는 걸로...(그냥 가장 가까운 지수승으로 return)
        # 그리고 opeartor마다 heuristic한 tuning을 나누지 않았기 때문에 오류가 날 가능성이 높다. user_autotune안에서 뭔가 reduction, pointwise 같은 것을 구분해야 할 것으로보인다.
        x_candidate =[]
        y_candidate =[]
        z_candidate =[]
        RBLOCK1 = 0
        total_configs = []
        if 'y' in self.numels:
           x_candidate = self.get_tile_config_candidates(self.numels['x'])
           y_candidate = self.get_tile_config_candidates(self.numels['y'])
           for x in x_candidate:
               for y in y_candidate:
                   total_configs.append({
                       'kwargs': {'XBLOCK': x, 'YBLOCK': y}, 'num_warps':2, 'num_stages':2})
           
        elif 'r' in self.numels:    
            x_candidate = self.get_tile_config_candidates(self.numels['x'])
            RBLOCK1 = 2 ** math.ceil(math.log2(self.numels['r']))
            for x in x_candidate:
                total_configs.append({
                       'kwargs': {'XBLOCK': x}, 'num_warps':2, 'num_stages':2})
        
        #x 만 있게되는 경우(tensor shape이 scalar인 경우)
        else :
            x_candidate = self.get_tile_config_candidates(self.numels['x'])
            for x in x_candidate:
                total_configs.append({
        ...
        # 이제 위에서 만든 configuration을 user_autotune안의 configs에 넣어줘야 한다.
        # 혹은 위에서 tile에 대해서 모두 검사해서 같은 grid size를 가지는 config만 넣어주는 방식?
        # 원래라면 propagate한 결과를 넘겨주어서 해야하는데
        

        if config.use_user_codegen:
            heuristics_line = f"""
                @triton_heuristics.user_autotune(
                    configs={total_configs!r},
